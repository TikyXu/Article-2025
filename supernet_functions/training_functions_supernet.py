...
class TrainerSupernet:
    ...
    def __init__(self, device_dis, device_gen, logger, writer, run_time, lookup_table, class_number_count):
        ...
        self.best_discriminator_for_generator = None # Discriminator最高Accuracy子网络，用于验证Generator生成准确率
        self.best_discriminator_accuracy = 0.0       # Discriminator最高Accuracy
    ...
    def train_loop(self, train_loader, test_loader, generator_supernet, discriminator_supernet):
        ...
        self.logger.info(f'(0).Discriminator {final_max_discriminator_code} [{pow(final_max_discriminator_latency, -1):e}, {final_max_accu:f}]')

        # TopK采样
        for i in range(pow(topk_dis, len(SEARCH_SPACE_TRANSFORMER["input_shape"]))-1):
            # 采样TopK个体,且规避重复
            while True:
                final_topk_discriminator, final_topk_discriminator_latency, final_topk_discriminator_code = self._sample(discriminator_supernet, mode='TopK', k=topk_dis)
            ...
            end_time_dis = time.time()
            self.logger.info(f'({i+1}).Discriminator {final_topk_discriminator_code} [{pow(final_topk_discriminator_latency, -1):e}, {final_topk_accu:f}]')
        
        for dis in final_dis_population.fronts:
            self._model_save(model=dis.model, gen_or_dis='Discriminator',
        ...
        max_sampled_gen_images_output, max_sampled_pe_code_fake = self.best_discriminator_for_generator(max_sampled_gen_images, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
        # Accuracy
        ...
        _, max_sampled_pe_code_validate = self.best_discriminator_for_generator(validate_image, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
        ...
        self.logger.info(f'(0).Generator {final_max_generator_code} [{generator_max_sampled_model_diversity:f}, {generator_max_sampled_model_accuracy:f}]')

        # TopK采样
        for i in range(pow(topk_dis, len(SEARCH_SPACE_TRANSFORMER["input_shape"]))-1):
            # 采样TopK个体,且规避重复
            while True:
                final_topk_generator, _, final_topk_generator_code = self._sample(generator_supernet, mode='TopK', k=topk_gen)
            ...
            end_time_gen = time.time()
            self.logger.info(f'({i+1}).Generator {final_topk_generator_code} [{final_topk_generator_diversity:f}, {final_topk_generator_accuracy:f}]')

        for gen in final_gen_population.fronts:
            self._model_save(model=gen.model, gen_or_dis='Generator',
    ...
    def _training_step(self, weight_or_theta, generator_supernet, discriminator_supernet, loader, test_loader, epoch):
        ...
        for image, label in loader:
            ...
            self.logger.info(f'Dis weights Iter:{iteration+1}, Accu:{dis_accuracy:f}, Loss:{dis_loss:.4f}')
            ...
            if weight_or_theta=='theta':                
                ...
                # 从population_dis非支配解集中获取最佳Accuracy的个体
                # 非支配解集个体按照Accuracy降序排列
                population_dis_objectives = np.array(population_dis.objectives)[population_dis.sorting[0]]
                sorted_idx = np.argsort(-population_dis_objectives[:, 1])
                
                if self.best_discriminator_for_generator is None:                    
                    self.best_discriminator_for_generator =  population_dis.fronts[sorted_idx[0]].model
                    self.best_discriminator_accuracy = population_dis.fronts[sorted_idx[0]].objectives[1]
                else:
                    if population_dis.fronts[sorted_idx[0]].objectives[1] > self.best_discriminator_accuracy:
                        self.best_discriminator_for_generator = population_dis.fronts[sorted_idx[0]].model
                        self.best_discriminator_accuracy = population_dis.fronts[sorted_idx[0]].objectives[1]
            ...
            generate_fake_image_origin, latency_to_accumulate_gen = generator_supernet(noise, fake_label_one_hot_dis, self.temperature, latency_to_accumulate_gen, True)
            generate_fake_image = generate_fake_image_origin.view(batch_size, 1, -1).cuda(non_blocking=True)
            
            discriminator_supernet.eval()
            ...
            # 原始R3GAN Generator的损失，只有softplus，没有R1+R2，改进BRGAN引入了R2，以改进不平衡数据集中判别器压制生成器
            # fake_loss_dis = nn.CrossEntropyLoss()(outs_fake_gen-mean_real_gen, label_gen)
            fake_loss_dis = nn.CrossEntropyLoss()(outs_fake_gen-mean_real_gen, fake_label_gen)

            # real_loss_dis = nn.CrossEntropyLoss()(outs_real_gen-mean_fake_gen, fake_label_gen)
            r2_penalty_gen = self._zero_centered_gradient_penalty(noise, generate_fake_image_origin)
            
            # gen_loss = (fake_loss_dis).mean()            
            gen_loss = (fake_loss_dis).mean() + ((gamma / 2) * r2_penalty_gen).mean()
            ...
            with torch.no_grad():
                # 生成测试样本
                generate_image_test, _ = generator_supernet(noise, fake_label_one_hot_dis, self.temperature, latency_to_accumulate_gen, True)
                generate_image_test = generate_image_test.view(batch_size, 1, -1).cuda(non_blocking=True)
                outs_test, _ = discriminator_supernet(generate_image_test, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), True)
            ...
            self.logger.info(f'Gen weights Iter:{iteration+1}, Accu:{gen_accuracy:f}, Loss:{gen_loss:.4f}')
            ...
            if weight_or_theta=='theta':
                ...
                if epoch == CONFIG_SUPERNET['train_settings']['train_thetas_from_the_epoch'] and iteration == 0:
                    for i in range(self.sample_number):
                        ...
                        random_sampled_gen_images_output, random_sampled_pe_code_fake = self.best_discriminator_for_generator(random_sampled_gen_images, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
                        # Accuracy
                        ...
                        _, random_sampled_pe_code_validate = self.best_discriminator_for_generator(validate_image, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
                    ...
                    for individual_gen_code in self.last_iteration_code_gen:
                        ...
                        survival_gen_images_output, survival_pe_code_fake = self.best_discriminator_for_generator(survival_gen_images, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
                        # Accuracy
                        ...
                        _, survival_pe_code_validate = self.best_discriminator_for_generator(validate_image, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
                    ...
                    for _ in range(int(1/4*self.sample_number)):
                        ...
                        topk_sampled_gen_images_output, topk_sampled_pe_code_fake = self.best_discriminator_for_generator(topk_sampled_gen_images, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
                        # Accuracy
                        ...
                        _, topk_sampled_pe_code_validate = self.best_discriminator_for_generator(validate_image, self.temperature, Variable(torch.Tensor([[0.0]]), requires_grad=True), False)
    ...
    def _intermediate_stats_logging(self, model_name, train_kind, accuracy, step, epoch, len_loader, time_consumption):        
        if (step > 0) and ((step + 1) % self.print_freq == 0) or step == len_loader - 1:
            self.logger.info(
                ...
                ": Epo:[{}/{}], Iter:[{}/{}], Accuracy:{:f}, Time:{:.4f}".format(
