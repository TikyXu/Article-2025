...
class TrainerArch:
    ...
    def train_loop(self, train_loader, valid_loader, model):
        ...
        print(f'Best accuracy:{best_top1}, epoch:{best_epoch+1}')
    ...
    def _train(self, loader, model, epoch):
        ...
        for step, (X, y) in enumerate(loader):
            X, y = X.view(X.shape[0], -1, X.shape[1]).cuda(non_blocking=True), y.cuda(non_blocking=True)
    ...
    def _intermediate_stats_logging(self, outs, y, loss, step, epoch, N, len_loader, val_or_train):
        prec1, prec3 = accuracy(outs, y, topk=(1, 0))
